...
- [x] **Real charge-contact signal in the serial protocol.** `STATUS` payload extended to 4 bytes (`watchdog_ok, blade_running, error_flags, charging`); firmware reads a new `CHARGE_DETECT_PIN` (`SensorReader::read_charging()`, active LOW, mirrors `LIFT_PIN`) and Python `decode_status`/`HardwareInterface.on_status` expose `charging`. `mower/main.py` feeds it through `LatestCharging` into `DockingManager`'s `charging_source` — no more `lambda: False` stub. **Still hardware bring-up:** `CHARGE_DETECT_PIN` (defined as pin 8 in `mower_firmware.ino`) is a placeholder — the physical charging-dock contact circuit doesn't exist yet (depends on "mount the marker"/dock build above), so the pin number and polarity must be revisited once that circuit is designed, and the firmware needs reflashing to a real Teensy (not verifiable in this dev environment). Tests: `tests/hal/test_protocol.py` (`TestTelemetryDecoding`), `tests/hal/test_hardware_interface.py::test_telemetry_callback_on_status_includes_charging`, `tests/executive/test_docking_runtime.py::TestLatestCharging`.
